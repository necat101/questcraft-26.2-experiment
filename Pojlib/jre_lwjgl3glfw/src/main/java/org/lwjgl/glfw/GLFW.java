...
public class GLFW
{
    ...
    private static int mGLFWClientApi = GLFW_OPENGL_API;
    ...
    public static final class Functions {
        ...
        GetNativeWindow = apiGetFunctionAddress(GLFW, "pojavGetNativeWindow"),
        ReleaseNativeWindow = apiGetFunctionAddress(GLFW, "pojavReleaseNativeWindow"),
    ...
    public static void glfwTerminate() {
        ...
        for (int i = 0; i < mGLFWWindowMap.size(); i++) {
            long window = mGLFWWindowMap.keyAt(i);
            if (mGLFWWindowMap.valueAt(i).nativeSurface) {
                invokePV(window, Functions.ReleaseNativeWindow);
            }
        }
        mGLFWWindowMap.clear();
        mainContext = 0;
    ...
    private static long mglfwCreateWindow(int width, int height, CharSequence title, long monitor, long share) {
        boolean nativeSurface = mGLFWClientApi == GLFW_NO_API;
        long ptr = nativeSurface ? invokeP(Functions.GetNativeWindow) : nglfwCreateContext(share);
        if (ptr == 0) {
            return 0;
        }
        ...
        win.title = title;
        win.nativeSurface = nativeSurface;
        win.windowAttribs.put(GLFW_RESIZABLE, GLFW_FALSE);
        win.windowAttribs.put(GLFW_CLIENT_API, mGLFWClientApi);
        ...
        mainContext = nativeSurface ? 0 : ptr;
    ...
    public static void glfwDefaultWindowHints() {
        ...
        mGLFWClientApi = GLFW_OPENGL_API;
    }
