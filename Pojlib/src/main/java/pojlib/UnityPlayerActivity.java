...
public class UnityPlayerActivity extends ActivityGroup implements IUnityPlayerLifecycleEvents, GrabListener
{
    ...
    private SurfaceView mMinecraftSurfaceView;
    private final Object mMinecraftSurfaceLock = new Object();
    private Surface mMinecraftSurface;
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        mMinecraftSurfaceView = new SurfaceView(this);
        mMinecraftSurfaceView.getHolder().setFormat(PixelFormat.RGBA_8888);
        mMinecraftSurfaceView.getHolder().addCallback(new SurfaceHolder.Callback() {
            @Override
            public void surfaceCreated(SurfaceHolder holder) {
                updateMinecraftSurface(holder.getSurface());
            }
            ...
            public void surfaceDestroyed(SurfaceHolder holder) {
                updateMinecraftSurface(null);
            }
        });
        mMinecraftSurfaceView.setAlpha(0.0f);
        mMinecraftSurfaceView.setZOrderOnTop(false);
        rootView.addView(mMinecraftSurfaceView, new FrameLayout.LayoutParams(
                FrameLayout.LayoutParams.MATCH_PARENT,
                FrameLayout.LayoutParams.MATCH_PARENT));
        rootView.addView(mUnityPlayer, new FrameLayout.LayoutParams(
                FrameLayout.LayoutParams.MATCH_PARENT,
                FrameLayout.LayoutParams.MATCH_PARENT));
    ...
    private void updateMinecraftSurface(Surface surface) {
        synchronized (mMinecraftSurfaceLock) {
            mMinecraftSurface = surface != null && surface.isValid() ? surface : null;
            mMinecraftSurfaceLock.notifyAll();
        }
    }

    public Surface getMinecraftSurface() {
        synchronized (mMinecraftSurfaceLock) {
            Surface surface = mMinecraftSurface;
            if (surface == null && mMinecraftSurfaceView != null) {
                surface = mMinecraftSurfaceView.getHolder().getSurface();
                if (surface != null && surface.isValid()) {
                    mMinecraftSurface = surface;
                    return surface;
                }
            }
            ...
            while ((mMinecraftSurface == null || !mMinecraftSurface.isValid()) &&
                ...
                try {
                    mMinecraftSurfaceLock.wait(remaining);
                } catch (InterruptedException e) {
                    Thread.currentThread().interrupt();
                    return null;
                }
            }
            return mMinecraftSurface != null && mMinecraftSurface.isValid() ? mMinecraftSurface : null;
        }
    }
    ...
    public static String installLWJGL(Activity activity) throws IOException {
        ...
        byte[] lwjglAsset = FileUtil.loadFromAssetToByte(activity, "lwjgl/lwjgl-glfw-classes.bin");
